...
public class RestAssuredExampleTestCases extends TestBase {
	...
	public void extractingResponseUsingJsonPath(){
        String request = tcData.get(_testCaseName).get("http://fakeapi.jsonparseronline.com/users/1");
    ...
    public void verifyResponsesUsingRootJSON(){
        ...
        given().
                get(tcData.get(_testCaseName).get("http://fakeapi.jsonparseronline.com/users/1")).
                then().
                root("RestResponse.result").
                body("id", equalTo(tcData.get(_testCaseName).get("id"))). // Use equalTo
                body("address", is(Integer.parseInt(tcData.get(_testCaseName).get("Id")))).	 // Instead of using 'equalTo' we can use 'is' also
    ...
    given().get("http://fakeapi.jsonparseronline.com/users/1").then().body(JSONObject);
